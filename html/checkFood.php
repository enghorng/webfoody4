...
<body>
    ...
    <!-- Food-Detail-Body -->
      ...
      <a href="admin-request.php"> <button type="button" class="btn btn-success" value="Delete"><<</button></a>
      <div class="row"> <br> </div>
        <div class="row">
          ...
          <a href="edit-food.php?food-id=<?php echo $foodCode ?>">
            <button type="button" class="btn btn-success"><i class="fa fa-edit"></i></button>
                </a>
            <div class="col-2 align-self-end"> 
            <select class="form-control" style="font-family: KhmerOSbattambang;" id="dropdown-status">
                    <option value="2">ការស្នើសុំ</option>
                    <option value="1">យល់ព្រម</option>
                    <option value="3">បដិសេធ</option>
                </select>
                    </div>
                    </div>
      </div>  
    ...
    <script>
      $(document).ready(function () {
        $("#dropdown-status").change(function(){
          var category = $("#dropdown-status").val();
          $.ajax({    //create an ajax request to display.php
            type: "GET",
            url: "../php/changeStatus.php?food-id=<?php echo $foodCode; ?>",             
            dataType: "html",  
              //expect html to be returned  
              data: {code:''+category},       
            success: function(data){                    
                $("#statuschanged").html(data); 
            }
          });
        });
      });
