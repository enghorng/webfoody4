<?php
    ...
    if($all==''){
        ...
        $foodSql = $con->query("SELECT * FROM `vuserfood` WHERE `Category` = 'Drink' and `Country`='".$country."' and `FoodType`='".$type."' ORDER BY FoodDate DESC");
        // echo "SELECT * FROM `Sweetvuserfood` WHERE `Category` = 'Food' and `Country`='".$country."' and `FoodType`='".$type."' ORDER BY FoodDate DESC";      
    }
    else if($country!='' && $type==''){
        $foodSql = $con->query("SELECT * FROM `vuserfood` WHERE `Category` = 'Drink' and `Country`='".$country."' ORDER BY FoodDate DESC");         

    }
    else if($country=='' && $type!=''){
        $foodSql = $con->query("SELECT * FROM `vuserfood` WHERE `Category` = 'Drink' and `FoodType`='".$type."' ORDER BY FoodDate DESC");         
